...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR    lpCmdLine, int nCmdShow)
{
	...
	try
	{
		...
		// 编译Shader创建渲染管线状态对象
		{
			...
			StringCchPrintf(pszShaderFileName, MAX_PATH, _T("%s\\PlacedTexture\\Shader\\Texture.hlsl"), pszAppPath);
		...
		// 创建纹理的默认堆
		{
			...
			////只有当Type是D3D12_HEAP_TYPE_CUSTOM时我们才能指定后面两个参数的值
			stTextureHeapDesc.Properties.CPUPageProperty = D3D12_CPU_PAGE_PROPERTY_UNKNOWN;	//CPU对这个堆内存的访问特性
			stTextureHeapDesc.Properties.MemoryPoolPreference = D3D12_MEMORY_POOL_UNKNOWN;	//指定在共享内存还是显存中存储
			//拒绝放置渲染目标纹理和深度蜡板缓冲纹理以及普通缓冲，实际就只是用来摆放普通纹理
			//代码中使用的两个拒绝（Deny）类型必须要一起设置，单独设置一个是不被允许的
		...
		// 使用“定位方式”创建顶点缓冲，使用与上传纹理数据缓冲相同的一个上传堆
		{
			...
			{
				{ { -0.25f * fAspectRatio, -0.25f * fAspectRatio, 0.0f, 1.0f}, { -2.0f, 2.0f } },	// Bottom left.
				{ { -0.25f * fAspectRatio, 0.25f * fAspectRatio, 0.0f, 1.0f}, { -2.0f, -2.0f } },	// Top left.
				{ { 0.25f * fAspectRatio, -0.25f * fAspectRatio, 0.0f, 1.0f}, { 2.0f, 2.0f } },	// Bottom right.
				{ { 0.25f * fAspectRatio, 0.25f * fAspectRatio, 0.0f, 1.0f}, { 2.0f, -2.0f } },		// Top right.
			};
		...
		// 创建采样器堆 和 各种采样器
		{
			...
			// Sampler 3
			//超出范围的纹理坐标将被强制限制在 [0, 1] 范围内（即所有大于1的坐标为1，小于0的坐标为0）
			...
			// Sampler 5
			//超出范围时，只会镜像一次，之后超出范围的纹理坐标将被强制限制在 [0, 1] 范围内
