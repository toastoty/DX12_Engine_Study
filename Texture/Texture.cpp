...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR    lpCmdLine, int nCmdShow)
{
	...
	try
	{
		...
		// 编译Shader创建渲染管线状态对象
		{
			...
			StringCchPrintf(pszShaderFileName, MAX_PATH, _T("%s\\Texture\\Shader\\Texture.hlsl"), pszAppPath);
		...
		{
			// 定义正方形的3D数据结构
			Toast_VERTEX stTriangleVertices[] =
			{
			...
			stVertexBufferView.StrideInBytes = sizeof(Toast_VERTEX);
		...
		// 使用WIC创建并加载一个2D纹理
		{
			...
			StringCchPrintfW(pszTexcuteFileName, MAX_PATH, _T("%s\\Assets\\bear.jpg"), pszAppPath);
			...
			if (!InlineIsEqualGUID(wpf, tgFormat))
			{
				...
				// 调用QueryInterface方法获得对象的 位图数据源接口(IWICFormatConverter : public IWICBitmapSource)
				GRS_THROW_IF_FAILED(pIConverter.As(&pIBMP));
			}
			else
			{
				//图片数据格式不需要转换，直接获取其 位图数据源接口
...
// 使用 CD3DX12_RESOURCE_DESC::Tex2D 简化结构体的初始化
			stTextureDesc = CD3DX12_RESOURCE_DESC::Tex2D(
				stTextureFormat,    // 纹理格式
				nTextureW,          // 宽度
				nTextureH,          // 高度
				1,                  // 数组大小，如果不是纹理数组，则为 1
				1,                  // mip 层级数量
				1,                  // SampleCount，默认为 1
				0,                  // SampleQuality，默认为 0
				D3D12_RESOURCE_FLAG_NONE,   // 资源标志
				D3D12_TEXTURE_LAYOUT_UNKNOWN,   // 纹理布局，默认为 UNKNOWN
				0);                 // 对于纹理堆的对齐要求，默认为 0
			...
			stBufDesc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;	//对于上传堆来说，其类型都必须是Buffer类型
			...
			{
				memcpy(pDestSlice + static_cast<SIZE_T>(stTxtLayouts.Footprint.RowPitch) * y	//目标地址
					, pSrcSlice + static_cast<SIZE_T>(nPicRowPitch) * y		//源地址
					, nPicRowPitch);	//每行的字节数
			}
			//取消映射 对于易变的数据如每帧的变换矩阵等数据，可以偷懒不用Unmap了，
			...
			//stResBar.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;		//资源屏障的类型为状态转换
			//stResBar.Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
			//stResBar.Transition.pResource = pITexture.Get();	//要进行状态转换的资源
			//stResBar.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_DEST;	//转换前的资源状态为复制目标状态
			//stResBar.Transition.StateAfter = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;	//转换后的资源状态为像素着色器资源状态
			//stResBar.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;	//指定要转换的子资源，这里是所有子资源

			//pICMDList->ResourceBarrier(1, &stResBar);
			auto transitionBarrier = CD3DX12_RESOURCE_BARRIER::Transition(pITexture.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
			pICMDList->ResourceBarrier(1, &transitionBarrier);
