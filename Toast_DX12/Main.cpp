...
#include <dxgidebug.h>
#endif
#include <comutil.h> // 包含 _com_error 类
#include <comdef.h>  // 添加 comdef.h 头文件
#include "d3dx12.h"
...
#define TS_WND_CLASS_NAME _T("TS Game Window Class")
#define TS_WND_TITLE _T("TS DirectX12 Trigger Sample")
#define TS_THROW_IF_FAILED(hr) {HRESULT _hr = (hr);if (FAILED(_hr)){ throw CGRSCOMException(_hr); }}
...
int APIENTRY _tWinMain(
	...
	try
	{
		...
		{
			TS_THROW_IF_FAILED(HRESULT_FROM_WIN32(GetLastError()));
		}
		...
		// 创建窗口
		{
			...
			wcex.lpszClassName = TS_WND_CLASS_NAME;
			...
			hWnd = CreateWindowW(TS_WND_CLASS_NAME
				, TS_WND_TITLE
				, dwWndStyle
				, posX
				, posY
		...
		// 创建DXGI Factory对象
		{
			TS_THROW_IF_FAILED(CreateDXGIFactory2(nDXGIFactoryFlags, IID_PPV_ARGS(&pIDXGIFactory5)));
			// 关闭ALT+ENTER键切换全屏的功能，因为我们没有实现OnSize处理，所以先关闭
			TS_THROW_IF_FAILED(pIDXGIFactory5->MakeWindowAssociation(hWnd, DXGI_MWA_NO_ALT_ENTER));
		...
		// 枚举适配器，并选择合适的适配器来创建3D设备对象
		{
			...
			// 创建D3D12.1的设备
			TS_THROW_IF_FAILED(D3D12CreateDevice(pIAdapter1.Get(), emFeatureLevel, IID_PPV_ARGS(&pID3D12Device4)));

			TCHAR pszWndTitle[MAX_PATH] = {};
			TS_THROW_IF_FAILED(pIAdapter1->GetDesc1(&stAdapterDesc));
		...
		// 创建直接命令队列
		{
			...
			TS_THROW_IF_FAILED(pID3D12Device4->CreateCommandQueue(&stQueueDesc, IID_PPV_ARGS(&pICMDQueue)));
		}
		...
		// 创建交换链
		{
			...
			TS_THROW_IF_FAILED(pIDXGIFactory5->CreateSwapChainForHwnd(
			...
			TS_THROW_IF_FAILED(pISwapChain1.As(&pISwapChain3));
			...
			//创建RTV(渲染目标视图)描述符堆(这里堆的含义应当理解为数组或者固定大小元素的固定大小显存池)
			{
				...
				TS_THROW_IF_FAILED(pID3D12Device4->CreateDescriptorHeap(
					&stRTVHeapDesc
			...
			//创建RTV的描述符
			{
				...
				{
					TS_THROW_IF_FAILED(pISwapChain3->GetBuffer(i
		...
		// 创建一个空的根描述符，也就是默认的根描述符
		{
			...
			TS_THROW_IF_FAILED(D3D12SerializeRootSignature(
				&stRootSignatureDesc
				, D3D_ROOT_SIGNATURE_VERSION_1
				, &pISignatureBlob
				, &pIErrorBlob));

			TS_THROW_IF_FAILED(pID3D12Device4->CreateRootSignature(0
		...
		// 创建命令列表分配器
		{
			TS_THROW_IF_FAILED(pID3D12Device4->CreateCommandAllocator(
				D3D12_COMMAND_LIST_TYPE_DIRECT
				, IID_PPV_ARGS(&pICMDAlloc)));

			// 创建图形命令列表
			TS_THROW_IF_FAILED(pID3D12Device4->CreateCommandList(
				0
				, D3D12_COMMAND_LIST_TYPE_DIRECT
				, pICMDAlloc.Get()
				, pIPipelineState.Get()
		...
		// 编译Shader创建渲染管线状态对象
		{
			...
			StringCchPrintf(pszShaderFileName
				, MAX_PATH
				//没改路径毁了我的DX12
				, _T("%s\\Toast_DX12\\Shader\\shaders.hlsl")
				, pszAppPath);

			TS_THROW_IF_FAILED(D3DCompileFromFile(pszShaderFileName
				, nullptr
				, nullptr
				, "VSMain"
				, "vs_5_0"
			...
			TS_THROW_IF_FAILED(pID3D12Device4->CreateGraphicsPipelineState(&stPSODesc, IID_PPV_ARGS(&pIPipelineState)));
		}
		...
		// 创建顶点缓冲
		{
			...
			//以下所有属性都可以使用D3D12_RESOURCE_XXX_UNKNOWN 作为未指定参数
			//资源的维度：缓冲区 D3D12_RESOURCE_DIMENSION_BUFFER、1D纹理 D3D12_RESOURCE_DIMENSION_TEXTURE1D、
			// 2D纹理 D3D12_RESOURCE_DIMENSION_TEXTURE2D、3D纹理 D3D12_RESOURCE_DIMENSION_TEXTURE3D
			stResSesc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
			// 行主要布局：数据按行排列，每行连续存储 D3D12_TEXTURE_LAYOUT_ROW_MAJOR
			// 64KB未定义重排布局，数据以64KB为单位进行分块（数据可以按不同方式组织）：D3D12_TEXTURE_LAYOUT_64KB_UNDEFINED_SWIZZLE
			// 64KB标准重排布局，数据以64KB为单位进行分块（数据以标准方式进行组织）：D3D12_TEXTURE_LAYOUT_64KB_STANDARD_SWIZZLE
			stResSesc.Layout = D3D12_TEXTURE_LAYOUT_ROW_MAJOR;
			// 标志资源的用途等
			// D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET：允许资源作为渲染目标。
			// D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL：允许资源作为深度模板缓冲区。
			...
			// 缓冲区资源为1
			stResSesc.Height = 1;
			//对于纹理资源：如果纹理是一个立方体贴图，则表示立方体贴图中立方体的数量。
			// 如果纹理是一个 3D 纹理，则表示纹理的深度，即立体纹理的层数。
			// 如果纹理是一个 2D 数组纹理，则表示纹理数组中纹理的数量。
			// 对于缓冲区资源：表示缓冲区中元素的数量。
			...
			//SampleDesc为多重采样属性
			// Count为采样数 Quality为质量级别
			// 对于缓冲区资源 SampleDesc.Count=1 Sample.Quality=0 
			stResSesc.SampleDesc.Count = 1;
			stResSesc.SampleDesc.Quality = 0;

			//CreateCommittedResource为内部同步函数，即函数返回时，实际的资源已经分配好了
			TS_THROW_IF_FAILED(pID3D12Device4->CreateCommittedResource(
				&stHeapProp,
			...
			TS_THROW_IF_FAILED(pIVertexBuffer->Map(
				0
				, &stReadRange
		...
		{
			TS_THROW_IF_FAILED(pID3D12Device4->CreateFence(0
		...
		TS_THROW_IF_FAILED(pICMDList->Close());
		...
		while (!bExit)
		{
			...
			case 0:
			{
				...
				//命令分配器先Reset一下
				TS_THROW_IF_FAILED(pICMDAlloc->Reset());
				//Reset命令列表，并重新指定命令分配器和PSO对象
				TS_THROW_IF_FAILED(pICMDList->Reset(pICMDAlloc.Get(), pIPipelineState.Get()));
				...
				//提交画面
				TS_THROW_IF_FAILED(pISwapChain3->Present(1, 0));

				//开始同步GPU与CPU的执行，先记录围栏标记值
				const UINT64 n64CurrentFenceValue = n64FenceValue;
				TS_THROW_IF_FAILED(pICMDQueue->Signal(pIFence.Get(), n64CurrentFenceValue));
				n64FenceValue++;
				TS_THROW_IF_FAILED(pIFence->SetEventOnCompletion(n64CurrentFenceValue, hEventFence));
			}
			break;
			case 1:
			{//处理消息
	...
	catch (CGRSCOMException& e)
	{
		...
		// 将错误信息显示出来
		MessageBox(nullptr, errMsg, _T("COM Exception"), MB_OK | MB_ICONERROR);
